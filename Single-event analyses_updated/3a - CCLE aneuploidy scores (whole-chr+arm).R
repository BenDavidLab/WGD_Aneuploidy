...
data <- read.csv("C:/Users/kiwii/OneDrive/Documents/Kavya/Uri/Work/Tasks/CCLE analysis/Prelim/anp_scores_samplewise_no-ke97_wca.tsv", sep = "\t", header = T)


input_box_1_fil_2 <- data.frame(count_anp_fil = data$Score, type_fil = data$Type, wgd_status_fil = data$`WGD.status`)
#write.table(input_box_1_fil_2, file="Table-CCLE-Pan cancer aneuploidy scores_no-ke97.tsv", sep = "\t")
...
sig_types <- c("BONE", "BREAST", "CENTRAL_NERVOUS_SYSTEM", "HAEMATOPOIETIC_AND_LYMPHOID_TISSUE",
               "KIDNEY", "LARGE_INTESTINE","LIVER", "LUNG", "OVARY", "PANCREAS", "SKIN", 
               "STOMACH", "UPPER_AERODIGESTIVE_TRACT", "URINARY_TRACT")
...
pdf(paste0(ROOT_DIR, "/", "CCLE-pan cancer aneuploidy scores_no-ke97_names1_qvals_wca.pdf"), width = 13, height = 7)
par(mar = c(14.2,4,1.5,1.5))
boxplot(count_anp_fil~wgd_status_fil*type_fil, col=color_vec_trans, data=input_box_1_fil_2, las = 1, ylab = "Number of aneuploidies per sample", cex.lab = 1, cex.axis = 0.8, xlab = "", cex.main = 0.9, main = "Pan-cancer cell line aneuploidy scores", outline = FALSE, at = VEC, ylim = c(0,40), mar = c(8,4,1.5,1.5), names = box_labels)
#textbox(c(15,50), -10, textbox_labels, box = F, justify = "l")
